...
const SearchSection = ({ setDraggedTweet }) => {
    ...
    const searchTweets = async () => {
        try {
            setTweets([])
            setLoading(true)
            const response = await fetch(`https://global-relay-server.herokuapp.com/search/${debouncedQuery.length > 0 ? debouncedQuery : "apple"}`)
        ...
        <Col md={6}>
            ...
            <div className="search-section position-relative ">
                ...
                <div id="search-bar-parent">
                    <input data-testid="search-bar" id="search-bar" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Find a tweet ... " />
